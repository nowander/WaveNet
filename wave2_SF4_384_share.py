...
from Fourth.ACCoNet_main.model.ResNet50 import Backbone_ResNet50_in3
from backbone.wavemlp import WaveMLP_M,WaveMLP_S
from backbone.wavemlp import WaveMLP_M,WaveMLP_S
import numpy as np
...
class SF4_Stu(nn.Module):
    def load_pre(self):
    # def load_pre(self, pre_model):
        self.encoderR.load_state_dict(torch.load('/media/sunfan/date/Paper_4/WaveMLP_S.pth'), strict=False)
        # self.encoderR.load_state_dict(torch.load(pre_model)['model'], strict=False)
        print(f"RGB SwinTransformer loading pre_model")# ${pre_model}")
        # self.encodert.load_state_dict(torch.load('/media/sunfan/date/Paper_4/WaveMLP_S.pth'), strict=False)
        # # self.encodert.load_state_dict(torch.load(pre_model)['model'], strict=False)
        # print(f"Depth SwinTransformer loading pre_model")# ${pre_model}")
    def __init__(self, channel=32):
        super(SF4_Stu, self).__init__()
        #Backbone model
        # ---- ResNet50 Backbone ----
        # (
        #     self.encoder1,
        ...
        # self.mi_level1 = Mutual_info_reg1(64, 64, 6)
        # self.mi_level2 = Mutual_info_reg1(64, 64, 6)
        # self.mi_level3 = Mutual_info_reg1(64, 64, 6)
        # self.mi_level4 = Mutual_info_reg1(64, 64, 6)
    ...
    def forward(self, x_rgb,x_thermal):
        ...
        # print('x2', x2.shape)
        # print('x3', x3.shape)
        # x0     torch.Size([4, 64, 96, 96])
        # x1     torch.Size([4, 128, 48, 48])
        # x2     torch.Size([4, 320, 24, 24])
        # x3     torch.Size([4, 512, 12, 12])
        ...
        # mer_cros4 = self.upsample2(mer_cros4)
        m4 = torch.cat((mer_cros4,x4_ACCoM),dim=1)
        mer_cros3 = self.PATM3(m4)
        # mer_cros3 = self.upsample2(mer_cros3)
        m3 = torch.cat((mer_cros3, x3_ACCoM), dim=1)
        mer_cros2 = self.PATM2(m3)
        m2 = torch.cat((mer_cros2, x2_ACCoM), dim=1)
        # mer_cros2 = self.upsample2(mer_cros2)
        mer_cros1 = self.PATM1(m2)
        # print('mer_cros1', mer_cros1.shape)
        # print('mer_cros2', mer_cros2.shape)
        # print('mer_cros3', mer_cros3.shape)
        # print('mer_cros4', mer_cros4.shape)
        # mer_cros1        torch.Size([4, 64, 192, 192])
        # mer_cros2        torch.Size([4, 128, 96, 96])
        # mer_cros3        torch.Size([4, 320, 48, 48])
        # mer_cros4        torch.Size([4, 512, 24, 24])
        s1 = self.up1(self.S1(mer_cros1))
        s2 = self.up2(self.S2(mer_cros2))
        s3 = self.up3(self.S3(mer_cros3))
        s4 = self.up3(self.S4(mer_cros4))
        # s1, s2, s3, s4 = self.decoder_rgb(x5_ACCoM, x4_ACCoM, x3_ACCoM, x2_ACCoM)
        # print('s1', s1.shape)
        # print('s1', s2.shape)
        # print('s1', s3.shape)
        # print('s1', s4.shape)
        ...
        # lat_loss0, z0_rgb, z0_depth = self.mi_level1(x_loss0, y_loss0)
        # lat_loss1, z1_rgb, z1_depth = self.mi_level2(x_loss1, y_loss1)
        # lat_loss2, z2_rgb, z2_depth = self.mi_level3(x_loss2, y_loss2)
        # lat_loss3, z3_rgb, z3_depth = self.mi_level4(x_loss3, y_loss3)
        ...
        # return s1, s2, s3,  self.sigmoid(s1), self.sigmoid(s2), self.sigmoid(s3),edge,lat_loss
        return s1, s2, s3, s4, self.sigmoid(s1), self.sigmoid(s2), self.sigmoid(s3), self.sigmoid(s4),edge,lat_loss#, self.sigmoid(s5)
####################
#[Statistics Information]
#FLOPs: 63.996G
#Params: 80.677M
#fps:  4.222972972972973
 ####################
if __name__=='__main__':
    ...
    CalParams(net, image, image)
    """修改权重
      from config2 import opt
    save_path = opt.save_path
    net = SF4_Stu().cuda(0)
    net.load_state_dict(
        torch.load('/media/sunfan/date/Paper_4/Wave/PTH_RGBT/Wave_Swin_384_Stu_val1000_RGBT/RES34_1_best_mae_test.pth'),strict=False)
    net.eval()
    with torch.no_grad():
        torch.save(net.state_dict(), save_path + 'RES34_1_best_mae_test.pth')
